    ...
    <div class="wc-shell">
        <div class="wc-header">
            ...
            <div class="wc-header-right">
                <nav class="wc-nav">
                    <a href="<?php echo esc_url(admin_url('admin.php?page=webchanges-ai-connector-mcp-skills')); ?>"><?php esc_html_e('Skills', 'webchanges-ai-connector-mcp'); ?></a>
                    <a href="<?php echo esc_url(admin_url('admin.php?page=webchanges-ai-connector-mcp-images')); ?>"><?php esc_html_e('Images', 'webchanges-ai-connector-mcp'); ?></a>
                    <a href="<?php echo esc_url(admin_url('admin.php?page=webchanges-ai-connector-mcp-abilities')); ?>"><?php esc_html_e('Abilities', 'webchanges-ai-connector-mcp'); ?></a>
                    <a href="https://shahbazdev.com/" target="_blank" rel="noopener"><?php esc_html_e('Author', 'webchanges-ai-connector-mcp'); ?></a>
                </nav>
                <div class="wc-status-pill <?php echo $enabled ? 'on' : ''; ?>">
                    <span class="wc-status-dot"></span>
                    <?php echo $enabled ? esc_html__('Connector active', 'webchanges-ai-connector-mcp') : esc_html__('Connector inactive', 'webchanges-ai-connector-mcp'); ?>
                </div>
            </div>
        </div>
        ...
        <?php if (!$abilities_api_ok): ?>
            <div class="wc-card">
                <div class="wc-notice wc-notice-error" style="margin:0;">
                    <strong><?php esc_html_e('Missing dependency:', 'webchanges-ai-connector-mcp'); ?></strong>
                    <?php esc_html_e('Install and activate the "Abilities API" WordPress plugin first.', 'webchanges-ai-connector-mcp'); ?>
        ...
        <?php if ($abilities_api_ok && !$mcp_adapter_ok): ?>
            <div class="wc-card">
                <div class="wc-notice wc-notice-error" style="margin:0;">
                    <strong><?php esc_html_e('MCP Adapter library missing.', 'webchanges-ai-connector-mcp'); ?></strong>
                    <?php esc_html_e('The plugin\'s vendor/ directory is incomplete — re-install from the official zip.', 'webchanges-ai-connector-mcp'); ?>
                </div>
            </div>
        <?php endif; ?>
...
<?php
        ...
        <details class="wc-card wc-card-collapsible wc-mt-4"<?php echo (!$enabled || $plaintext !== null || $create_error !== null) ? ' open' : ''; ?>>
            <summary class="wc-card-title"><?php esc_html_e('Setup the connector', 'webchanges-ai-connector-mcp'); ?></summary>

            <div class="wc-step">
                <div class="wc-step-num">1</div>
                <div class="wc-step-body">
                    <h3 class="wc-step-title"><?php esc_html_e('Enable the connector', 'webchanges-ai-connector-mcp'); ?></h3>
                    <p class="wc-step-desc"><?php esc_html_e('Connects this WordPress site to any MCP-compatible AI client (Claude Code, Claude Desktop, Cursor, and others). When active, the connected agent can edit posts, blocks, Bricks pages, media, SEO, taxonomies, menus, users, and more on this site.', 'webchanges-ai-connector-mcp'); ?></p>
                    <form method="post" class="wc-row">
                        ...
                        <button type="submit" class="wc-btn <?php echo $enabled ? 'wc-btn-danger' : 'wc-btn-primary'; ?>">
                            <?php echo $enabled
                                ? esc_html__('Disable connector', 'webchanges-ai-connector-mcp')
                                : esc_html__('Enable connector', 'webchanges-ai-connector-mcp');
                            ?>
                        </button>
                    </form>
                </div>
            ...
            <div class="wc-step">
                ...
                <div class="wc-step-body">
                    <h3 class="wc-step-title"><?php esc_html_e('Create an application password', 'webchanges-ai-connector-mcp'); ?></h3>
                    <p class="wc-step-desc"><?php esc_html_e('MCP clients authenticate with this site over HTTP using an application password. Each password is shown once — copy it immediately. Existing passwords stay valid even if you leave this page.', 'webchanges-ai-connector-mcp'); ?></p>
                    ...
                    <?php if ($plaintext !== null): ?>
                        <div class="wc-pw-box">
                            ...
                            <button type="button" class="wc-btn" onclick="wcCopy('wc-new-pw', this)"><?php esc_html_e('Copy', 'webchanges-ai-connector-mcp'); ?></button>
                            <span class="wc-pw-warn"><?php esc_html_e('Saved? It won\'t be shown again.', 'webchanges-ai-connector-mcp'); ?></span>
                        </div>
                    <?php endif; ?>

                    <form method="post" class="wc-row">
                        <?php wp_nonce_field('webchanges_connector_create_password'); ?>
                        <input type="text" name="password_name" class="wc-input" style="max-width:340px;" placeholder="<?php esc_attr_e('Label (optional) — e.g. "Claude Desktop"', 'webchanges-ai-connector-mcp'); ?>" maxlength="70">
                        <button type="submit" name="webchanges_connector_create_password" class="wc-btn wc-btn-primary" <?php echo !$app_pw_status['available'] ? 'disabled' : ''; ?>>
                            <?php esc_html_e('Create application password', 'webchanges-ai-connector-mcp'); ?>
                        </button>
                    </form>
                    ...
                    <?php if ($existing !== []): ?>
                        <table class="wc-table wc-mt-4">
                            <thead><tr>
                                <th><?php esc_html_e('Name', 'webchanges-ai-connector-mcp'); ?></th>
                                <th><?php esc_html_e('Created', 'webchanges-ai-connector-mcp'); ?></th>
                                <th><?php esc_html_e('Last used', 'webchanges-ai-connector-mcp'); ?></th>
                                <th></th>
                            </tr></thead>
                            <tbody>
                            <?php foreach ($existing as $pw): ?>
                                <tr>
                                    ...
                                    <td class="muted"><?php echo esc_html(($pw['last_used'] ?? 0) ? wp_date('Y-m-d H:i', (int) $pw['last_used']) : __('Never', 'webchanges-ai-connector-mcp')); ?></td>
                                    <td style="text-align:right;">
                                        <form method="post" style="margin:0;display:inline-block;" onsubmit="return confirm('<?php echo esc_js(__('Revoke this password?', 'webchanges-ai-connector-mcp')); ?>');">
                                            <?php wp_nonce_field('webchanges_connector_admin'); ?>
                                            ...
                                            <button type="submit" class="wc-btn wc-btn-danger" style="padding:6px 12px;font-size:12px;"><?php esc_html_e('Revoke', 'webchanges-ai-connector-mcp'); ?></button>
                                        </form>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
            ...
            <div class="wc-step">
                ...
                <div class="wc-step-body">
                    <h3 class="wc-step-title"><?php esc_html_e('Connect your MCP client', 'webchanges-ai-connector-mcp'); ?></h3>
                    <p class="wc-step-desc"><?php esc_html_e('Pick your client below — copy the snippet and run/paste it. The stdio bridges all require Node.js (provides npx).', 'webchanges-ai-connector-mcp'); ?></p>
                    ...
                    <div class="wc-code-block">
                        ...
                        <button type="button" class="wc-copy-btn" onclick="wcCopyConfig(this)"><?php esc_html_e('Copy', 'webchanges-ai-connector-mcp'); ?></button>
                    </div>
        ...
        <details class="wc-card wc-card-collapsible wc-mt-4"<?php echo is_wp_error($image_gen_saved) || $image_gen_saved === true ? ' open' : ''; ?>>
            <summary class="wc-card-title"><?php esc_html_e('AI Image Generation (optional)', 'webchanges-ai-connector-mcp'); ?></summary>
            <p style="font-size:13.5px;color:var(--wc-fg-muted);margin-bottom:6px;">
                <?php esc_html_e('Connect an image-generation API so agents can auto-create featured images instead of pulling from free stock libraries. Keys are stored encrypted-at-rest in your WordPress database and never leave this site.', 'webchanges-ai-connector-mcp'); ?>
            </p>
            <p style="font-size:12px;color:var(--wc-fg-faint);margin-bottom:18px;">
                <?php esc_html_e('Get a key:', 'webchanges-ai-connector-mcp'); ?>
            ...
            <?php elseif ($image_gen_saved === true): ?>
                <div class="wc-notice wc-notice-info"><?php esc_html_e('Image generation settings saved.', 'webchanges-ai-connector-mcp'); ?></div>
            <?php endif; ?>
            ...
            <form method="post" class="wc-stack">
                ...
                <div style="display:grid;grid-template-columns:repeat(auto-fit, minmax(220px, 1fr));gap:14px;">
                    <div>
                        <label style="display:block;font-size:11px;text-transform:uppercase;letter-spacing:0.6px;color:var(--wc-fg-faint);font-weight:600;margin-bottom:6px;"><?php esc_html_e('Default provider', 'webchanges-ai-connector-mcp'); ?></label>
                    ...
                    <div>
                        <label style="display:block;font-size:11px;text-transform:uppercase;letter-spacing:0.6px;color:var(--wc-fg-faint);font-weight:600;margin-bottom:6px;"><?php esc_html_e('Default model', 'webchanges-ai-connector-mcp'); ?></label>
                    ...
                    <div>
                        <label style="display:block;font-size:11px;text-transform:uppercase;letter-spacing:0.6px;color:var(--wc-fg-faint);font-weight:600;margin-bottom:6px;"><?php esc_html_e('Default size', 'webchanges-ai-connector-mcp'); ?></label>
                ...
                <div>
                    <label style="display:block;font-size:11px;text-transform:uppercase;letter-spacing:0.6px;color:var(--wc-fg-faint);font-weight:600;margin-bottom:6px;"><?php esc_html_e('Default style hint (appended to auto-prompts)', 'webchanges-ai-connector-mcp'); ?></label>
                ...
                <div style="margin-top:6px;">
                    <div style="font-size:11px;text-transform:uppercase;letter-spacing:0.6px;color:var(--wc-fg-faint);font-weight:600;margin-bottom:10px;"><?php esc_html_e('API keys', 'webchanges-ai-connector-mcp'); ?></div>
                    <?php foreach ($image_gen_providers as $slug => $meta):
                    ...
                    ?>
                        <details class="wc-api-key-row">
                            <summary>
                                ...
                                <?php else: ?>
                                    <span style="font-size:11px;color:var(--wc-fg-faint);">○ <?php esc_html_e('Not configured', 'webchanges-ai-connector-mcp'); ?></span>
                                <?php endif; ?>
                            </summary>
                            <div class="wc-api-key-body">
                                <input type="password" autocomplete="new-password" name="<?php echo esc_attr($slug); ?>_api_key" class="wc-input" placeholder="<?php echo $current !== '' ? esc_attr__('Leave blank to keep existing key — type to replace, or "clear" to remove', 'webchanges-ai-connector-mcp') : esc_attr__('Paste your API key', 'webchanges-ai-connector-mcp'); ?>">
                            </div>
                        </details>
                    <?php endforeach; ?>
                </div>
                ...
                <div style="margin-top:8px;">
                    <button type="submit" class="wc-btn wc-btn-primary"><?php esc_html_e('Save image generation settings', 'webchanges-ai-connector-mcp'); ?></button>
                </div>
            </form>
        </details>
        <?php endif; ?>
        ...
        <details class="wc-card wc-card-collapsible wc-mt-4"<?php echo is_wp_error($stock_saved) || $stock_saved === true ? ' open' : ''; ?>>
            <summary class="wc-card-title"><?php esc_html_e('Stock Photos (free fallback)', 'webchanges-ai-connector-mcp'); ?></summary>
            <p style="font-size:13.5px;color:var(--wc-fg-muted);margin-bottom:6px;">
                <?php esc_html_e('Connect Pexels / Unsplash / Pixabay so agents can pull commercially-licensed photos when no AI image provider is configured. All three are free APIs. Keys are stored encrypted-at-rest in your WordPress database and never leave this site.', 'webchanges-ai-connector-mcp'); ?>
            </p>
            <p style="font-size:12px;color:var(--wc-fg-faint);margin-bottom:18px;">
                <?php esc_html_e('Get a key:', 'webchanges-ai-connector-mcp'); ?>
            ...
            <?php elseif ($stock_saved === true): ?>
                <div class="wc-notice wc-notice-info"><?php esc_html_e('Stock photo settings saved.', 'webchanges-ai-connector-mcp'); ?></div>
            <?php endif; ?>
            ...
            <form method="post" class="wc-stack">
                ...
                <div style="display:grid;grid-template-columns:repeat(auto-fit, minmax(220px, 1fr));gap:14px;">
                    <div>
                        <label style="display:block;font-size:11px;text-transform:uppercase;letter-spacing:0.6px;color:var(--wc-fg-faint);font-weight:600;margin-bottom:6px;"><?php esc_html_e('Default provider', 'webchanges-ai-connector-mcp'); ?></label>
                        <select name="default_provider" class="wc-input">
                            <option value="" <?php selected((string) ($stock_settings['default_provider'] ?? ''), ''); ?>><?php esc_html_e('— Auto (first configured) —', 'webchanges-ai-connector-mcp'); ?></option>
                    ...
                    <div>
                        <label style="display:flex;align-items:center;gap:8px;font-size:11px;text-transform:uppercase;letter-spacing:0.6px;color:var(--wc-fg-faint);font-weight:600;margin-top:24px;cursor:pointer;">
                            ...
                            <?php esc_html_e('Fall back to stock when no AI provider is configured', 'webchanges-ai-connector-mcp'); ?>
                        </label>
                    </div>
                </div>
                ...
                <div style="margin-top:6px;">
                    ...
                    ?>
                        <details class="wc-api-key-row">
                            ...
                            <div class="wc-api-key-body">
                                <input type="password" autocomplete="new-password" name="<?php echo esc_attr($field_name); ?>" class="wc-input" placeholder="<?php echo $current !== '' ? esc_attr__('Leave blank to keep existing key — type to replace, or "clear" to remove', 'webchanges-ai-connector-mcp') : esc_attr__('Paste your API key', 'webchanges-ai-connector-mcp'); ?>">
                            </div>
                        </details>
                    <?php endforeach; ?>
                </div>
                ...
                <div style="margin-top:8px;">
                    <button type="submit" class="wc-btn wc-btn-primary"><?php esc_html_e('Save stock photo settings', 'webchanges-ai-connector-mcp'); ?></button>
                </div>
            </form>
        </details>
        <?php endif; ?>
        ...
        <details class="wc-card wc-card-collapsible wc-mt-4">
            <summary class="wc-card-title"><?php /* translators: %d is the number of registered abilities */ printf(esc_html__('Registered abilities · %d', 'webchanges-ai-connector-mcp'), (int) count($abilities_ours)); ?></summary>

            <div class="wc-stat-grid" style="margin-bottom:18px;">
                <div class="wc-stat"><div class="wc-stat-value"><?php echo (int) count($abilities_ours); ?></div><div class="wc-stat-label"><?php esc_html_e('Abilities live', 'webchanges-ai-connector-mcp'); ?></div></div>
                <div class="wc-stat"><div class="wc-stat-value"><?php echo (int) count($abilities_by_category); ?></div><div class="wc-stat-label"><?php esc_html_e('Categories', 'webchanges-ai-connector-mcp'); ?></div></div>
                <div class="wc-stat"><div class="wc-stat-value"><?php echo (int) count($existing); ?></div><div class="wc-stat-label"><?php esc_html_e('App passwords', 'webchanges-ai-connector-mcp'); ?></div></div>
                <div class="wc-stat"><div class="wc-stat-value" style="font-size:14px;letter-spacing:0;font-weight:600;"><?php echo esc_html(get_bloginfo('version')); ?> · PHP <?php echo esc_html(PHP_VERSION); ?></div><div class="wc-stat-label"><?php esc_html_e('Runtime', 'webchanges-ai-connector-mcp'); ?></div></div>
            </div>
        ...
        <details class="wc-card wc-card-collapsible wc-mt-4">
            <summary class="wc-card-title"><?php esc_html_e('Connection & capabilities', 'webchanges-ai-connector-mcp'); ?></summary>
        <div class="wc-grid wc-grid-2" style="gap:32px;">
            <div>
                <div class="wc-card-title"><?php esc_html_e('Connection details', 'webchanges-ai-connector-mcp'); ?></div>
                <dl class="wc-kv">
                    <dt><?php esc_html_e('Endpoint', 'webchanges-ai-connector-mcp'); ?></dt>
                    <dd><?php echo esc_html($rest_url); ?></dd>
                    <dt><?php esc_html_e('Username', 'webchanges-ai-connector-mcp'); ?></dt>
                    <dd><?php echo esc_html($username); ?></dd>
                    <dt><?php esc_html_e('Auth', 'webchanges-ai-connector-mcp'); ?></dt>
                    <dd style="font-family:inherit;font-size:13px;color:var(--wc-fg-muted);"><?php esc_html_e('HTTP Basic with WordPress Application Password', 'webchanges-ai-connector-mcp'); ?></dd>
                    <dt><?php esc_html_e('Protocol', 'webchanges-ai-connector-mcp'); ?></dt>
                    <dd style="font-family:inherit;font-size:13px;color:var(--wc-fg-muted);"><?php esc_html_e('MCP 2025-03-26 over JSON-RPC / HTTP', 'webchanges-ai-connector-mcp'); ?></dd>
                </dl>
            </div>
            <div>
                <div class="wc-card-title"><?php esc_html_e('Site capabilities', 'webchanges-ai-connector-mcp'); ?></div>
                <dl class="wc-kv">
                    <?php
                    $caps = [
                    ...
                    foreach ($caps as $name => $on): ?>
                        ...
                        <dd style="font-family:inherit;font-size:13px;color:<?php echo $on ? 'var(--wc-success)' : 'var(--wc-fg-faint)'; ?>;">
                            <?php echo $on ? '● ' . esc_html__('Active', 'webchanges-ai-connector-mcp') : '○ ' . esc_html__('Not detected', 'webchanges-ai-connector-mcp'); ?>
                        </dd>
                    <?php endforeach; ?>
                </dl>
                ...
                <div style="display:flex;align-items:baseline;justify-content:space-between;margin-bottom:10px;">
                    <div style="font-size:10.5px;text-transform:uppercase;letter-spacing:0.8px;color:var(--wc-fg-faint);font-weight:600;"><?php esc_html_e('Image Generation', 'webchanges-ai-connector-mcp'); ?></div>
                    <?php if (!empty($img_settings['default_provider']) && ($img_settings[$img_settings['default_provider'] . '_api_key'] ?? '') !== ''): ?>
                        <div style="font-size:10.5px;color:var(--wc-fg-faint);">
                            <?php esc_html_e('default:', 'webchanges-ai-connector-mcp'); ?>
                ...
                <dl class="wc-kv">
                    <?php foreach ($img_providers_status as $name => $on): ?>
                        <dt><?php echo esc_html($name); ?></dt>
                        <dd style="font-family:inherit;font-size:13px;color:<?php echo $on ? 'var(--wc-success)' : 'var(--wc-fg-faint)'; ?>;">
                            <?php echo $on ? '● ' . esc_html__('Connected', 'webchanges-ai-connector-mcp') : '○ ' . esc_html__('Not connected', 'webchanges-ai-connector-mcp'); ?>
                        </dd>
                    <?php endforeach; ?>
                </dl>
                <?php endif; ?>
                ...
                <div style="margin:18px 0 12px;height:1px;background:rgba(255,255,255,0.06);"></div>
                <div style="display:flex;align-items:baseline;justify-content:space-between;margin-bottom:10px;">
                    <div style="font-size:10.5px;text-transform:uppercase;letter-spacing:0.8px;color:var(--wc-fg-faint);font-weight:600;"><?php esc_html_e('Stock Photos', 'webchanges-ai-connector-mcp'); ?></div>
                    <?php if ($stk_default !== ''): ?>
                        <div style="font-size:10.5px;color:var(--wc-fg-faint);">
                            <?php esc_html_e('default:', 'webchanges-ai-connector-mcp'); ?>
                            ...
                            <?php if (!empty($stk['fallback_for_ai'])): ?>
                                · <span style="color:var(--wc-fg-muted);"><?php esc_html_e('AI fallback on', 'webchanges-ai-connector-mcp'); ?></span>
                            <?php endif; ?>
                        </div>
                    <?php endif; ?>
                </div>
    ...
    (function () {
        ...
        window.wcCopyConfig = function (btn) {
            navigator.clipboard.writeText(document.getElementById('wc-config-code').textContent).then(function () {
                var o = btn.textContent; btn.textContent = '<?php echo esc_js(__('Copied!', 'webchanges-ai-connector-mcp')); ?>';
...
function webchanges_connector_handle_image_gen_save()
{
    ...
    if (!current_user_can('manage_options')) {
        return new \WP_Error('forbidden', __('You do not have permission.', 'webchanges-ai-connector-mcp'));
    }
    check_admin_referer('webchanges_connector_image_gen');
    if (!function_exists('webchanges_image_gen_save_settings')) {
        return new \WP_Error('not_loaded', __('Image generation module not loaded.', 'webchanges-ai-connector-mcp'));
    }
    $patch = [];
...
function webchanges_connector_handle_stock_save()
{
    ...
    if (!function_exists('webchanges_stock_save_settings')) {
        return new \WP_Error('not_loaded', __('Stock photo module not loaded.', 'webchanges-ai-connector-mcp'));
    }
    $patch = [];
...
function webchanges_connector_handle_create_password()
{
    ...
    if (!class_exists('WP_Application_Passwords')) {
        return new \WP_Error('not_available', __('Application passwords are not available on this WordPress install.', 'webchanges-ai-connector-mcp'));
    }
...
function webchanges_connector_app_passwords_status(): array
{
    if (!function_exists('wp_is_application_passwords_available')) {
        return ['available' => false, 'message' => __('This WordPress version does not support application passwords.', 'webchanges-ai-connector-mcp')];
    }
    if (!wp_is_application_passwords_available()) {
        return ['available' => false, 'message' => __('Application passwords are disabled on this site. Enable them in wp-config.php or with the wp_is_application_passwords_available filter.', 'webchanges-ai-connector-mcp')];
    }
    if (!wp_is_application_passwords_available_for_user(get_current_user_id())) {
        return ['available' => false, 'message' => __('Application passwords are disabled for your user account.', 'webchanges-ai-connector-mcp')];
    }
    ...
    return [
        'claude-code' => [
            ...
            'hint' => sprintf(
                ...
                __('Run this command in your terminal. Then start a new Claude Code session — the abilities appear under <code>mcp__%s__</code>.', 'webchanges-ai-connector-mcp'),
                esc_html($mcp_name)
            ),
        ],
        'claude-desktop' => [
            'code' => $mcp_servers_json,
            'hint' => __('Add to <code>claude_desktop_config.json</code>: macOS <code>~/Library/Application Support/Claude/claude_desktop_config.json</code>, Windows <code>%APPDATA%\\Claude\\claude_desktop_config.json</code>. Restart Claude Desktop.', 'webchanges-ai-connector-mcp'),
        ],
        'cursor' => [
            'code' => $mcp_servers_json,
            'hint' => __('Add to <code>~/.cursor/mcp.json</code> (global) or <code>.cursor/mcp.json</code> (project).', 'webchanges-ai-connector-mcp'),
        ],
        'vscode' => [
            'code' => $vscode_servers_json,
            'hint' => __('Add to <code>.vscode/mcp.json</code> (workspace).', 'webchanges-ai-connector-mcp'),
        ],
        'webchanges' => [
            'code' => sprintf("URL:      %s\nUsername: %s\nPassword: %s", $rest_url, $username, $password),
            'hint' => __('Paste these three values into your MCP client to connect it to this site.', 'webchanges-ai-connector-mcp'),
        ],
    ];
}
