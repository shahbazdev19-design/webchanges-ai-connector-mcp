...
add_action('admin_menu', static function () {
    add_submenu_page(
        ...
        __('Skills', 'webchanges-ai-connector-mcp'),
        __('Skills', 'webchanges-ai-connector-mcp'),
        'manage_options',
        'webchanges-ai-connector-mcp-skills',
...
function webchanges_connector_handle_skills_admin()
{
    ...
    if ($action === 'save') {
        ...
        if ($macro_raw !== '') {
            ...
            if (!is_array($decoded)) {
                return new \WP_Error('bad_macro', __('Macro must be a valid JSON array, or left blank.', 'webchanges-ai-connector-mcp'));
            }
            $macro = $decoded;
        }
        $data = [
    ...
    if ($action === 'upload') {
        if (empty($_FILES['md']['tmp_name']) || !is_uploaded_file($_FILES['md']['tmp_name'])) { // phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized -- $_FILES tmp_name is a server-generated path handled by the WP upload flow
            return new \WP_Error('no_file', __('No file was uploaded.', 'webchanges-ai-connector-mcp'));
        }
        ...
        if ($content === '') {
            return new \WP_Error('empty', __('The uploaded file was empty.', 'webchanges-ai-connector-mcp'));
        }
...
function webchanges_connector_render_skills_page(): void
{
    ...
    $base_url = admin_url('admin.php?page=webchanges-ai-connector-mcp-skills');
    ...
    <div class="wc-shell">
        ...
        <?php elseif ($result === 'saved'): ?>
            <div class="wc-notice wc-notice-success"><?php esc_html_e('Skill saved.', 'webchanges-ai-connector-mcp'); ?></div>
        <?php elseif ($result === 'imported'): ?>
            <div class="wc-notice wc-notice-success"><?php esc_html_e('Skill imported from markdown.', 'webchanges-ai-connector-mcp'); ?></div>
        <?php elseif ($result === 'deleted'): ?>
            <div class="wc-notice wc-notice-success"><?php esc_html_e('Custom skill deleted.', 'webchanges-ai-connector-mcp'); ?></div>
        <?php elseif ($result === 'enabled'): ?>
            <div class="wc-notice wc-notice-success"><?php esc_html_e('Skill enabled.', 'webchanges-ai-connector-mcp'); ?></div>
        <?php elseif ($result === 'disabled'): ?>
            <div class="wc-notice wc-notice-success"><?php esc_html_e('Skill disabled on this site.', 'webchanges-ai-connector-mcp'); ?></div>
        <?php endif; ?>

        <div class="wc-notice wc-notice-info">
            <?php esc_html_e('Skills are reusable specialist playbooks the agent loads on demand. Bundled skills ship with the plugin and update across every site; custom skills live only here. After changes, reconnect your AI client so it sees the new state.', 'webchanges-ai-connector-mcp'); ?>
        </div>

        <div class="wc-grid wc-grid-2">
            <div>
                <div class="wc-card">
                    <div class="wc-card-title"><?php esc_html_e('Your custom skills', 'webchanges-ai-connector-mcp'); ?> <span class="wc-count"><?php echo (int) count($custom); ?></span></div>
                    <?php if ($custom === []): ?>
                        <div class="wc-empty"><?php esc_html_e('No custom skills yet. Add one on the right, or upload a .md file. To ship a skill to every site, add it to the plugin repo under /skills.', 'webchanges-ai-connector-mcp'); ?></div>
                        ...
                        <div class="wc-row" style="<?php echo $on ? '' : 'opacity:0.5;'; ?>">
                            <div class="wc-row-main">
                                <div class="wc-row-name">
                                    ...
                                    <?php if (!empty($s['macro'])): ?><span class="wc-chip wc-chip-run"><?php esc_html_e('runnable', 'webchanges-ai-connector-mcp'); ?></span><?php endif; ?>
                                    <?php if (!$on): ?><span class="wc-chip"><?php esc_html_e('disabled', 'webchanges-ai-connector-mcp'); ?></span><?php endif; ?>
                                </div>
                            ...
                            <div class="wc-row-actions">
                                <form method="post">
                                    ...
                                    <button class="wc-btn wc-btn-sm" type="submit"><?php echo $on ? esc_html__('Disable', 'webchanges-ai-connector-mcp') : esc_html__('Enable', 'webchanges-ai-connector-mcp'); ?></button>
                                </form>
                                <a class="wc-btn wc-btn-sm" href="<?php echo esc_url(add_query_arg('edit', $slug, $base_url)); ?>"><?php esc_html_e('Edit', 'webchanges-ai-connector-mcp'); ?></a>
                                <form method="post" onsubmit="return confirm('<?php echo esc_js(__('Delete this custom skill?', 'webchanges-ai-connector-mcp')); ?>');">
                                    <?php wp_nonce_field('webchanges_skills'); ?>
                                    <input type="hidden" name="webchanges_skills_action" value="delete">
                                    <input type="hidden" name="slug" value="<?php echo esc_attr($slug); ?>">
                                    <button class="wc-btn wc-btn-sm wc-btn-danger" type="submit"><?php esc_html_e('Delete', 'webchanges-ai-connector-mcp'); ?></button>
                                </form>
                            </div>
                        </div>
                ...
                <div class="wc-card">
                    <div class="wc-card-title"><?php esc_html_e('Bundled skills', 'webchanges-ai-connector-mcp'); ?> <span class="wc-count"><?php echo (int) count($bundled); ?></span></div>
                    <div class="wc-card-sub"><?php esc_html_e('Shipped with the plugin and version-controlled. Edit them in the GitHub repo under /skills, then release — they update on every site.', 'webchanges-ai-connector-mcp'); ?></div>
                    <?php if ($bundled === []): ?>
                        <div class="wc-empty"><?php esc_html_e('No bundled skills found.', 'webchanges-ai-connector-mcp'); ?></div>
                        ...
                        <div class="wc-row" style="<?php echo $on ? '' : 'opacity:0.5;'; ?>">
                            <div class="wc-row-main">
                                <div class="wc-row-name">
                                    <?php echo esc_html($s['name']); ?>
                                    <span class="wc-mono"><?php echo esc_html($slug); ?></span>
                                    <span class="wc-chip wc-chip-bundled"><?php esc_html_e('bundled', 'webchanges-ai-connector-mcp'); ?></span>
                                    <?php if (!empty($s['macro'])): ?><span class="wc-chip wc-chip-run"><?php esc_html_e('runnable', 'webchanges-ai-connector-mcp'); ?></span><?php endif; ?>
                                    <?php if (!$on): ?><span class="wc-chip"><?php esc_html_e('disabled', 'webchanges-ai-connector-mcp'); ?></span><?php endif; ?>
                                </div>
                                <div class="wc-row-desc"><?php echo esc_html($s['description']); ?></div>
            ...
            <div>
                <div class="wc-card">
                    <div class="wc-card-title"><?php echo $editing ? esc_html__('Edit custom skill', 'webchanges-ai-connector-mcp') : esc_html__('Add a custom skill', 'webchanges-ai-connector-mcp'); ?></div>
                    <form method="post">
                        <?php wp_nonce_field('webchanges_skills'); ?>
                        <input type="hidden" name="webchanges_skills_action" value="save">
                        <div class="wc-field">
                            <label class="wc-label"><?php esc_html_e('Name', 'webchanges-ai-connector-mcp'); ?></label>
                            <input class="wc-input" type="text" name="name" required value="<?php echo esc_attr($editing['name'] ?? ''); ?>">
                        </div>
                        <div class="wc-field">
                            <label class="wc-label"><?php esc_html_e('Slug (kebab-case)', 'webchanges-ai-connector-mcp'); ?></label>
                        ...
                        <div class="wc-field">
                            <label class="wc-label"><?php esc_html_e('Description', 'webchanges-ai-connector-mcp'); ?></label>
                            <input class="wc-input" type="text" name="description" value="<?php echo esc_attr($editing['description'] ?? ''); ?>" placeholder="<?php esc_attr_e('One action-first sentence the agent uses to decide when to load it.', 'webchanges-ai-connector-mcp'); ?>">
                        </div>
                        <div class="wc-field">
                            <label class="wc-label"><?php esc_html_e('Instructions (markdown)', 'webchanges-ai-connector-mcp'); ?></label>
                        ...
                        <details class="wc-adv"<?php echo (!empty($editing['macro'])) ? ' open' : ''; ?>>
                            <summary><?php esc_html_e('Advanced: runnable macro (JSON, optional)', 'webchanges-ai-connector-mcp'); ?></summary>
                        ...
                        <div style="margin-top:14px;display:flex;gap:8px;">
                            <button class="wc-btn wc-btn-primary" type="submit"><?php echo $editing ? esc_html__('Update skill', 'webchanges-ai-connector-mcp') : esc_html__('Add skill', 'webchanges-ai-connector-mcp'); ?></button>
                            <?php if ($editing): ?><a class="wc-btn" href="<?php echo esc_url($base_url); ?>"><?php esc_html_e('Cancel', 'webchanges-ai-connector-mcp'); ?></a><?php endif; ?>
                        </div>
                    </form>
                </div>

                <div class="wc-card">
                    <div class="wc-card-title"><?php esc_html_e('Upload a .md skill', 'webchanges-ai-connector-mcp'); ?></div>
                    <div class="wc-card-sub"><?php esc_html_e('Markdown file with frontmatter (name, description). Only upload skills from sources you trust.', 'webchanges-ai-connector-mcp'); ?></div>
                    <form method="post" enctype="multipart/form-data">
                        ...
                        <button class="wc-btn" type="submit"><?php esc_html_e('Upload .md', 'webchanges-ai-connector-mcp'); ?></button>
                    </form>
                </div>
            </div>
        </div>
