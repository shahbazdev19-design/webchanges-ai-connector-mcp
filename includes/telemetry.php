...
{
    // OPT-IN: off by default. No ping is ever sent unless the site owner turns
    // it on via the `webchanges_connector_telemetry_optin` option (or the filter).
    $optin = (bool) get_option('webchanges_connector_telemetry_optin', false);
    return (bool) apply_filters('webchanges_connector_telemetry_enabled', $optin);
}
