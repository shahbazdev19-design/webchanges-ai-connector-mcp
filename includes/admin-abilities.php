...
add_action('admin_menu', static function () {
    add_submenu_page(
        ...
        __('Abilities', 'webchanges-ai-connector-mcp'),
        __('Abilities', 'webchanges-ai-connector-mcp'),
        'manage_options',
        'webchanges-ai-connector-mcp-abilities',
...
function webchanges_connector_render_abilities_page(): void
{
    ...
    <div class="wc-shell">
        ...
        <?php elseif ($result === 'saved'): ?>
            <div class="wc-notice wc-notice-success"><?php esc_html_e('Abilities updated. Disabled abilities are no longer exposed to the agent — reconnect your AI client to refresh.', 'webchanges-ai-connector-mcp'); ?></div>
        <?php endif; ?>
        ...
        <form method="post" id="wc-abilities-form">
            ...
            <div class="wc-notice wc-notice-info" style="display:flex;align-items:center;justify-content:space-between;gap:16px;flex-wrap:wrap;">
                <span>
                    <strong style="color:var(--wc-fg);"><?php echo (int) $active; ?></strong> <?php esc_html_e('of', 'webchanges-ai-connector-mcp'); ?> <strong style="color:var(--wc-fg);"><?php echo (int) $total; ?></strong> <?php esc_html_e('abilities enabled. Untick any you do not want this site to expose, then Save.', 'webchanges-ai-connector-mcp'); ?>
                </span>
                <span style="display:flex;gap:8px;align-items:center;flex-wrap:wrap;">
                    <input type="search" id="wc-ability-filter" class="wc-input" style="max-width:240px;" placeholder="<?php esc_attr_e('Filter…', 'webchanges-ai-connector-mcp'); ?>">
                    <button type="button" class="wc-btn wc-btn-sm" data-wc-all="1"><?php esc_html_e('Enable all', 'webchanges-ai-connector-mcp'); ?></button>
                    <button type="button" class="wc-btn wc-btn-sm" data-wc-all="0"><?php esc_html_e('Disable all', 'webchanges-ai-connector-mcp'); ?></button>
                    <button type="submit" class="wc-btn wc-btn-primary wc-btn-sm"><?php esc_html_e('Save changes', 'webchanges-ai-connector-mcp'); ?></button>
                </span>
            </div>
                        ...
                        <label class="wc-row wc-ability" data-search="<?php echo esc_attr(strtolower($short . ' ' . $r['description'])); ?>" style="cursor:pointer;align-items:center;">
                            ...
                            <span class="wc-row-main">
                                <span class="wc-row-name">
                                    ...
                                    <?php if ($is_protected): ?><span class="wc-chip"><?php esc_html_e('always on', 'webchanges-ai-connector-mcp'); ?></span><?php endif; ?>
                                    <?php if ($is_dangerous): ?><span class="wc-chip" style="background:rgba(220,38,38,.15);color:#fca5a5;border-color:rgba(220,38,38,.4);" title="<?php esc_attr_e('High risk: grants code execution or arbitrary file changes. Off by default — enable only if you trust the connected client.', 'webchanges-ai-connector-mcp'); ?>">⚠ <?php esc_html_e('high risk — off by default', 'webchanges-ai-connector-mcp'); ?></span><?php endif; ?>
                                </span>
            ...
            <div style="margin-top:20px;"><button type="submit" class="wc-btn wc-btn-primary"><?php esc_html_e('Save changes', 'webchanges-ai-connector-mcp'); ?></button></div>
        </form>
    </div>
    <script>
    (function () {
